...
export function ProjectConversation({
  ...
  return (
    <section className="space-y-3.5">
      {/* One card, the way a ticket's thread is one card: the heading with what
          it holds on the right, and the rail under a faint divider. */}
      <div className="card overflow-hidden">
        <div className="flex h-[38px] items-center justify-between gap-3 px-4">
          <h2 className="label">{t.projects.conversation}</h2>
          <span className="text-text-3 text-sm">{t.projects.commentCount(comments.length)}</span>
        </div>

        {items.length === 0 ? (
          <p className="border-line text-text-3 border-t px-4 py-6 text-base">
            {t.projects.noComments}
          </p>
        ) : (
          <ol className="border-line [&>li+li]:border-line border-t [&>li+li]:border-t">
            {items.map((item) =>
              item.kind === "event" ? (
                <li
                  key={item.id}
                  className="group/act text-text-2 flex items-center gap-2.5 px-4 py-2.5 text-sm"
                >
...
/**
 * One message, flush beside its author's face — the shape a ticket's comments
 * have now that the thread is one card, so a person who writes in both places
 * is writing in the same place twice. No card of its own: it is already on a
 * card, and a second white box on a white box says nothing.
 */
function CommentRow({
  comment,
  stamp,
      ...
      <div className="group/comment flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="relative">
            ...
            {canDelete ? (
              <span className="bg-surface rounded-control absolute -top-4 right-0 z-10 flex shrink-0 opacity-0 shadow-[var(--shadow-float)] transition-opacity group-hover/comment:opacity-100 focus-within:opacity-100">
                <ConfirmDelete
